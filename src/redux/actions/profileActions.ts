...
// Interfacce Azioni
interface GetProfileLoadingAction {
  type: typeof GET_PROFILE_LOADING
}
...
interface PutProfileLoadingAction {
  type: typeof PUT_PROFILE_LOADING
}
interface PutProfileSuccessAction {
  type: typeof PUT_PROFILE_SUCCESS
  payload: Profile
}
interface PutProfileErrorAction {
  type: typeof PUT_PROFILE_ERROR
  payload: string
}
interface GetAllProfilesLoadingAction {
  type: typeof GET_ALL_PROFILES_LOADING
}
interface GetAllProfilesSuccessAction {
  type: typeof GET_ALL_PROFILES_SUCCESS
  payload: Profile[]
}
interface GetAllProfilesErrorAction {
  type: typeof GET_ALL_PROFILES_ERROR
  payload: string
}
interface UploadImageLoadingAction {
...
interface UploadImageErrorAction {
...
export type ProfileActions =
  ...
  | GetAllProfilesLoadingAction
  | GetAllProfilesSuccessAction
  | GetAllProfilesErrorAction
  | UploadImageLoadingAction
...
export const getAllProfilesAction = () => {
  return async (dispatch: Dispatch<ProfileActions>) => {
    ...
    try {
      ...
      dispatch({
        type: GET_ALL_PROFILES_ERROR,
        payload: errorMessage || "Errore nel caricamento dei profili",
      })
    }
  }
}
...
export const deleteProfileImage = () => {
  return async (dispatch: Dispatch<ProfileActions>) => {
    dispatch({ type: PUT_PROFILE_LOADING })
    try {
      const data = await customFetch<Profile>("profile/", "PUT", { image: "" })
      dispatch({ type: PUT_PROFILE_SUCCESS, payload: data })
      return data
    } catch (error) {
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Errore nella rimozione della foto"
      dispatch({ type: PUT_PROFILE_ERROR, payload: errorMessage })
      return null
    }
  }
